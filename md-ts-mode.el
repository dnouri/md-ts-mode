...
(defconst md-ts--character-reference-regexp
  (concat "&\\(?:#\\([0-9]\\{1,7\\}\\)"
          "\\|#[xX]\\([[:xdigit:]]\\{1,6\\}\\)"
          "\\|\\([[:alpha:]][[:alnum:]]*\\)\\);")
  "Regexp matching CommonMark character references with semicolons.")

(defconst md-ts--html5-character-reference-alist
  '(
    ("AElig" . "Æ")
    ("AMP" . "&")
    ("Aacute" . "Á")
    ...
    ("zwj" . "‍")
    ("zwnj" . "‌"))
  "HTML5 named character references accepted by CommonMark.
Only semicolon-terminated names are included, matching CommonMark
character reference syntax.  Generated from Python
`html.entities.html5` data.")

(defvar md-ts--character-reference-table nil
  "Hash table mapping named character references to decoded strings.")

(defun md-ts--character-reference-table ()
  "Return a hash table for named Markdown character references."
  (or md-ts--character-reference-table
      (setq md-ts--character-reference-table
            (let ((table (make-hash-table :test #'equal)))
              (dolist (entity md-ts--html5-character-reference-alist)
...
backslash escapes, and decode Markdown character references."
