...
- Hide-markup mode: ~md-ts-toggle-hide-markup~ hides delimiters
  (=#=, =*=, =~=, brackets, URLs) so only content is visible; visible
  link text remains activatable.
...
prose URLs, email addresses, and =mailto:= URIs.  Use =C-c C-o=
...
now; same-buffer fragment navigation is not implemented yet.  Escaped
