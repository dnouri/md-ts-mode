...
- libtree-sitter runtime 0.25 or later.  Older distro runtimes such as
  0.22 are not supported because they mis-handle disjoint parser ranges.
...
counts; ~make perf-smoke~ runs a tiny runtime smoke and fails when the
Markdown grammars are unavailable.
...
Tested on Emacs 29.4, 30.2, and development snapshot (31).
CI probes libtree-sitter 0.25.10 on those Emacs versions, and also
probes libtree-sitter 0.26 on snapshot.  CI intentionally does not run
against the GitHub image's default tree-sitter runtime when it is older
than the 0.25 minimum.
Emacs 29/30 with libtree-sitter 0.26+ is not currently a supported
combination for ~md-ts-mode~, because upstream Emacs only gained that
runtime compatibility later.
