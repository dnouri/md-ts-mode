...
Fragment navigation is deferred: local paths such as =file.md#heading=
open only =file.md=, and fragment-only links such as =#heading= are
unsupported and signal an error for now.  Escaped hashes such as
=a\#b.md= are literal filename characters.
