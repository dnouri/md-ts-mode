...
- Follow links: ~md-ts-browse-url-at-point~ (bound to ~C-c C-o~) opens
  the link URL at point, resolving it from the parse tree so it works
  even when hide-markup has hidden the URL.
