...
#+begin_src elisp
  (package-vc-install "https://github.com/dnouri/md-ts-mode")
#+end_src
...
Loading the package defines ~md-ts-mode~ and registers its grammar
recipes, but it does not change how Markdown files are chosen globally.

If you want ~md-ts-mode~ to handle Markdown explicitly, add one line to
your init file:
...
With a package install the helper is autoloaded; for the manual
~load-path~ setup above, keep the ~require~ line.

The helper is idempotent.  It adds a =.md= association, remaps
~markdown-mode~, and on Emacs 31 also prefers ~md-ts-mode~ over the
...
If only part of a file is highlighted, one of the grammars is
probably missing.
