...
~md-ts-mode~ buttonizes Markdown links, images, reference links,
reference-definition labels, URI autolinks, email autolinks, and bare
prose URLs, email addresses, and =mailto:= URIs.  Bare-link detection is
prose-only and skips code, HTML, parsed-link/autolink, and
reference-definition contexts.  Use =C-c C-o=
...
Emacs' mailto handler, and other URI schemes use ~browse-url~.  For
local paths such as =file.md#heading=, only the file part is opened for
now; same-buffer fragment navigation is not implemented yet.
Fragment-only links such as =#heading= are unsupported for now.  Escaped
hashes such as =a\#b.md= are literal filename characters.
