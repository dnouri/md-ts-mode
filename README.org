...
recipes, but it does not change how =.md= files or ~markdown-mode~ are
chosen globally.  Use ~M-x md-ts-mode~ in a buffer, or add your own
explicit file-association/remap settings in your init file if you want
~md-ts-mode~ to handle Markdown globally.
