...
Run ~make check~ before sending changes.  ~make perf~ runs advisory
link fontification benchmarks; compare =RESULT= lines from runs on the
same machine rather than treating them as pass/fail thresholds.  Benchmark
=RESULT= lines include tracked dirty/untracked state and link-artifact
counts.  ~make perf-smoke~ validates the local grammar and link-artifact
path only; CI, not this smoke target, covers the supported runtime matrix.
